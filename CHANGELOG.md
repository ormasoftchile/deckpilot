...
All notable changes to the Deckpilot extension will be documented in this file.
...
- **Live Preview enhancements**: cursor follow (caret in editor scrolls preview to matching slide), reverse sync (click a slide in preview to jump editor to its source line), `{{ENV_VAR}}` interpolation using the deck's resolved environment, and display of cached `render:command` output (preview still never executes commands).
