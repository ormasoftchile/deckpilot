 ...
 *
 * Framework-neutral: operates on the `TextDocument` duck type and returns plain
 * `DiagnosticResult[]`. Validation-rule NAME checks use the pure `isValidRule`
 * (backed by core's VALIDATION_RULES). On-disk / environment EXISTENCE checks are
 * delegated to an optional host-injected `EnvExistenceChecker` (see below).
 */
...
import {
  ...
  ACTION_SCHEMAS,
  ActionBlockRange,
} from '@deckpilot/core/models/actionSchema';
import { ActionType } from '@deckpilot/core/models/action';
import { isValidRule } from './validationRules';
import type { EnvExistenceChecker } from './envExistenceChecker';
...
export class ActionDiagnosticProvider {
  ...
  /**
   * Optional host-injected checker for env EXISTENCE validation (fs/PATH/etc.).
   *
   * RESERVED SEAM (Phase 1): the authoring diagnostics below validate env rule
   * *names* via the pure `isValidRule` but perform NO on-disk existence checks.
   * The extension still injects a real checker (backed by its `EnvRuleValidator`)
   * so a future LSP / preflight can perform real existence validation here
   * without the framework-neutral package touching `fs`/`child_process`.
   * It is intentionally unexercised today — see the Phase-1 decision note.
   */
  private readonly _existenceChecker?: EnvExistenceChecker;

  constructor(existenceChecker?: EnvExistenceChecker) {
    this._existenceChecker = existenceChecker;
  }

  /**
   * The host-injected env EXISTENCE checker, if one was provided.
   *
   * Exposed for the reserved seam: a future LSP / preflight step can retrieve
   * the checker to perform real on-disk existence validation. The Phase-1
   * authoring diagnostics do not call it. Returns `undefined` when the provider
   * was constructed without a checker (e.g. web editor / unit tests).
   */
  get existenceChecker(): EnvExistenceChecker | undefined {
    return this._existenceChecker;
  }
  ...
  private validateEnvFrontmatter(
    ...
    for (let idx = 0; idx < (fm['env'] as unknown[]).length; idx++) {
      ...
      // env-invalid-rule
      if (typeof validate === 'string' && !isValidRule(validate)) {
...
/**
 * Facade: compute diagnostics for a document without managing a provider
 * instance. Behavior is identical to
 * `new ActionDiagnosticProvider(existenceChecker).computeDiagnostics(document)`.
 */
export function getDiagnostics(
  document: TextDocument,
  existenceChecker?: EnvExistenceChecker,
): DiagnosticResult[] {
  return new ActionDiagnosticProvider(existenceChecker).computeDiagnostics(document);
}
