 ...
 *
 * Framework-neutral: operates on the `TextDocument` duck type, returns plain
 * `CompletionItem[]`. Host adapters (VS Code / browser editor) map these to
 * their own completion types.
 */
...
import type { TextDocument, Position } from './textDocument';

/**
 * Framework-neutral completion item. Host adapters map this onto their own
 * completion type (e.g. `vscode.CompletionItem`).
 */
...
export class ActionCompletionProvider {
   ...
   */
  private findFrontmatter(document: TextDocument): [number, number] | null {
...
/**
 * Facade: compute completion items for a document position without managing a
 * provider instance. Behavior is identical to
 * `new ActionCompletionProvider().provideCompletionItems(...)`.
 */
export function getCompletions(
  ...
  return new ActionCompletionProvider().provideCompletionItems(document, position, token, context);
}
