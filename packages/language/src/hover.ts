 ...
 *
 * Framework-neutral: operates on the `TextDocument` duck type, returns a plain
 * `HoverResult`. Host adapters (VS Code / browser editor) map it to their own
 * hover type.
 */
...
/**
 * Framework-neutral hover result (compatible with vscode.Hover).
 */
...
/**
 * Facade: compute a hover result for a document position without managing a
 * provider instance. Behavior is identical to
 * `new ActionHoverProvider().provideHover(...)`.
 */
export function getHover(
  ...
  return new ActionHoverProvider().provideHover(document, position, token);
}
