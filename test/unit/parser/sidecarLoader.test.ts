...
describe('sidecarLoader', () => {
  ...
  describe('resolveSidecarPath', () => {
    ...
    it('resolves companion .deck.yaml for a plain .md file', () => {
      const result = resolveSidecarPath('/foo/notes.md');
      expect(result).to.equal('/foo/notes.deck.yaml');
    });

    it('throws a clear error when the path does not end with .deck.md or .md', () => {
      expect(() => resolveSidecarPath('/foo/notes.txt')).to.throw(
        "resolveSidecarPath: expected a .md or .deck.md file path, got 'notes.txt'"
      );
    });

    it('throws when given a .deck.yaml path directly', () => {
      expect(() => resolveSidecarPath('/foo/demo.deck.yaml')).to.throw(
        "resolveSidecarPath: expected a .md or .deck.md file path, got 'demo.deck.yaml'"
      );
    });

    it('throws when given a bare filename with no extension', () => {
      expect(() => resolveSidecarPath('/foo/mydeck')).to.throw(
        "resolveSidecarPath: expected a .md or .deck.md file path, got 'mydeck'"
      );
    });
  });
  ...
  describe('sidecarExists', () => {
    ...
    it('returns true when .deck.yaml exists alongside a plain .md file', async () => {
      const plainMd = path.join(tmpDir, 'notes.md');
      const deckYaml = path.join(tmpDir, 'notes.deck.yaml');
      fs.writeFileSync(plainMd, '');
      fs.writeFileSync(deckYaml, '');

      const result = await sidecarExists(plainMd);
      expect(result).to.be.true;
    });
    ...
    it('propagates the error from resolveSidecarPath when path is not .deck.md or .md', async () => {
      let caught: Error | undefined;
      try {
        await sidecarExists('/foo/notes.txt');
      } catch (err) {
        caught = err as Error;
      }
      expect(caught).to.be.instanceOf(Error);
      expect(caught?.message).to.include('expected a .md or .deck.md file path');
    });
  });
